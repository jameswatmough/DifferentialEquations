...
A solution to a first order differential equation, $y' = f(x,y)$, is a function, $y(x)$, whose slope at $x$ is $f(x,y)$.  One way to visualize this is to view $f$ as a *direction field*.  That is, $f$ assigns a slope, or direction, to every point in the $x$-$y$ plane.  The figure below shows the direction field $f:(x,y)\to x+\sin y$.  

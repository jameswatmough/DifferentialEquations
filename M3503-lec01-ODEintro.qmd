...
Separating variables leads to $\int \frac{1}{y}\,dy = \int r\cos(t)\,dt +C$.
Integrating both sides leads to $\ln y = r\sin(t) + C$.
This can be solved for $y$ in terms of $t$.  $y(t) = e^{C+r\sin(t)}$.
Note the solution is usually given in the form $y(t) = Ae^{r\sin(t)}$.
Note that we must assume $y\ne0$ in order to perform the separation of 
variables.  However, it is easy to see by inspection that $y=0$ is a solution 
to the equation.
